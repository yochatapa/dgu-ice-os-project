...
void logIn()
{
    ...
    else
    {
        ...
        while(!suc)
        {
            ...
            if(strcmp(username,"plus")) //user�߰� x
            {
                 temp = root->lchild;
                 while(temp!=NULL) // user�� �ִ��� ������ �˻�
                 {
                     if(!strcmp(temp->name,username)) //user�� �ִٸ� suc = 1
                     {
                         suc = 1;
                         break;
                     }
...
void pwd(treeptr cur,treeptr rot)
{
    ...
    while(temptree != cur)
    {
        ...
        if(temptree != rot) printf("/%s",temptree->name);
        temptree = tempcur;
    }
    if(cur == rot) printf("/\n");
    else printf("/%s\n",cur->name);
    ...
    strcpy(svdata,temp->data);
            ...
            else //��� ���
            {
                ...
                else
                {
                    ...
                    {
                        if(token == NULL) // tempcur2�� NULL, ���̻� token�� ����
                        {
                            printf("mkdir :can't make %s: already have file\n",svdata);
                            return;
                        }
...
void makedirectory_p(nodeptr temp,char *permission)
{
    char svdata[100];
            ...
            if(temp->data[0] == '/') //���� ���
            {
                ...
                token = strtok(NULL,"/");
                while(token!=NULL) //�� ó�� tempcur = user
                {
...
void cat_path(nodeptr tempnode,treeptr temptree,int option_c,int option_n)
{
    char svdata[100];
            ...
            if(pathtype == 0) //���� ���
            {
                ...
                token = strtok(NULL,"/"); //token = ���� �� , temptree = user
                while(token!=NULL)
                {
                    temppath = token;
                    token = strtok(NULL,"/");
                    ...
                    else
                    {
                        ...
                        if(temptree2 == NULL)
                        {
                            if(token == NULL) //printf("make file2  %s/%s\n",temptree->name,temppath); //�̹� lchild�� �ٸ� ���� �Ǵ� ���� ����
                            {
